...
const SortingTable = () => {
    ...
    const { getTableProps, getTableBodyProps, footerGroups, headerGroups, rows, prepareRow, } = (0, react_table_1.useTable)({
        columns,
        data,
    ...
    return (<table {...getTableProps()}>
      <thead>
        {headerGroups.map((headerGroup, index) => (<tr {...headerGroup.getHeaderGroupProps()} key={index}>
            {headerGroup.headers.map((column, colIndex) => (<th {...column.getHeaderProps(column.getSortByToggleProps())} key={colIndex}>
                {column.id === "firstname" ? (<>
